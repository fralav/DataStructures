#ifndef GRAFO_GRAFO_LIST_H_
#define GRAFO_GRAFO_LIST_H_

#include "grafo.h"
#include "../lista/vector_list.h"
...
void GrafoList<E, P>::insArco(Nodo n1, Nodo n2, P peso) {
  ...
  I.peso = peso;
  I._to = n2;
  matrice[n1.getId()].archi.insert(I, matrice[n1.getId()].archi.first());
  archi++;
}
...
void GrafoList<E, P>::cancArco(Nodo n1, Nodo n2){
	typename VectorList<InfoArco<P>>::position p;
	p = matrice[n1.getId()].archi.first();
	bool trovato = false;
	while (!matrice[n1.getId()].archi.isEnd(p) && !trovato){
...
typename GrafoList<E, P>::ListaNodi GrafoList<E, P>::Adiacenti(Nodo n) const{
  ListaNodi list;
  typename VectorList<InfoArco<P>>::position p;
	p = matrice[n.getId()].archi.first();
  while (!matrice[n.getId()].archi.isEnd(p)){
		list.insert(new NodoG(matrice[n.getId()].archi.read(p)._to), list.first());
...
typename GrafoList<E, P>::ListaNodi GrafoList<E, P>::list_nodi() const {
  ...
  for (int i = 0; i < dimensione; i++)
    if (!matrice[i].vuoto)
      list.insert(new NodoG(i), list.first());
  return list;
}
